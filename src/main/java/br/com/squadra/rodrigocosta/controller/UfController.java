...
public class UfController {
    ...
    @GetMapping(value = "/uf")
    public ResponseEntity<?> listaUfsComParametros(@RequestParam(required = false) Long codigoUf, @RequestParam(required = false) String nome, @RequestParam(required = false) String sigla, @RequestParam(required = false) Long status) {
        if (codigoUf == null && nome == null && sigla == null && status == null) {
            ...
            if (listaUfsResponse.isEmpty()) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Não existe nenhuma UF cadastrada no banco de dados!");
            } else {
            ...
            if (listaUfsResponse.isEmpty()) {
                return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Não existe nenhuma UF cadastrada no banco de dados" + " que corresponde aos valores que foram passados!");
            } else if (codigoUf != null && nome == null && sigla == null && status == null) {
                return ResponseEntity.ok(listaUfsResponse.stream().findFirst().get());
            } else {
                return ResponseEntity.ok().body(listaUfsResponse);
            }
        }
